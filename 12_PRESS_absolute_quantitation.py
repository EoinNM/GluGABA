...
print '========================================================================================'
print '                            GluGABA - 12_PRESS_Absolute_Quantitation                    '
print '========================================================================================'

def absolute_quantitation(workspace, results):
    ...
    def calc_concentrations(LCM, GM, WM, CSF):
        ...
        #Gussew compartmentation equation to calculate absolute metabolite concentration via consideration of heterogenous tissue composition
        chet = (LCM) * (((GM * GM_h2o * r_h20_GM + WM * WM_h20 * r_h20_WM + CSF * CSF_h20 * r_h20_CSF) / 
        ...
        for se_voxel in PRESS_voxels:
            for day in days:
                for dtype in data_type:
                    ...
                    lcm = (os.path.join(workspace, 'RESULTS', 'PRESS', 'LCMODEL', se_voxel, day, '%s_%s_%s.csv'%(se_voxel, day, dtype)))
                    df = pd.read_csv(lcm, index_col = 0)
                    ...
                    df.to_csv(os.path.join(absolute_dir, 'ABS_%s_%s_%s.csv'%(se_voxel, day, dtype)))
...
absolute_quantitation(workspace, results)