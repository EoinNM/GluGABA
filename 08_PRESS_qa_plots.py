"""
Visual Assessment of the spectra, phase and frequency drift, and the voxel location in anatomical space
"""
import os
from os import *
        ...
        for day in days:
            for dtype in data_type:
                for se_voxel in PRESS_voxels:
                    ...
                    lcm_dir = os.path.join(workspace, subject, 'LCMODEL', day, se_voxel, dtype)
                    lcm_file = os.path.join(lcm_dir, se_voxel, '%s_%s_%s.pdf' %(subject, se_voxel, day))
                    qa_dir = mkdir_path(os.path.join(workspace, subject, 'QA_Plots', day, se_voxel, dtype))
                    twx_qa_dir = mkdir_path(os.path.join(workspace, subject, 'QA_Plots', day, se_voxel, 'TWIX'))
                    twx_out_dir = os.path.join(workspace, subject, 'SVS', day, se_voxel, 'TWIX', se_voxel, 'report', 'figs')
                ...
                T1 = os.path.join(workspace, subject, 'ANATOMICAL', day, 'ANATOMICAL.nii')
                ACC = os.path.join(workspace, subject, 'SVS', day, 'ACC', 'voxel_masks', 'ACC.rda_Mask.nii')
                M1 = os.path.join(workspace, subject, 'SVS', day, 'M1', 'voxel_masks', 'M1.rda_Mask.nii')
                M1_qa_dir = os.path.join(workspace, subject, 'QA_Plots', day, 'M1', dtype)
                ACC_qa_dir = os.path.join(workspace, subject, 'QA_Plots', day, 'ACC', dtype)
                ...
                report = canvas.Canvas(os.path.join(M1_qa_dir,'QC_REPORT_%s_M1.pdf'%(subject)), pagesize=(1280, 1556))
                ...
                M1_fig1 = os.path.join(workspace, subject, 'QA_Plots', day, 'M1', 'TWIX', 'alignAvgs_prePostFig.png')
                M1_fig2 = os.path.join(workspace, subject, 'QA_Plots', day, 'M1', 'TWIX', 'freqDriftFig.png')
                M1_fig3 = os.path.join(workspace, subject, 'QA_Plots', day, 'M1', 'TWIX', 'phaseDriftFig.png')
                ...
                report = canvas.Canvas(os.path.join(ACC_qa_dir,'QC_REPORT_%s_ACC.pdf'%(subject)), pagesize=(1280, 1556))
                report.setFont("Helvetica", 40)
                ...
                ACC_fig1 = os.path.join(workspace, subject, 'QA_Plots', day, 'ACC', 'TWIX', 'alignAvgs_prePostFig.png')
                ACC_fig2 = os.path.join(workspace, subject, 'QA_Plots', day, 'ACC', 'TWIX', 'freqDriftFig.png')
                ACC_fig3 = os.path.join(workspace, subject, 'QA_Plots', day, 'ACC', 'TWIX', 'phaseDriftFig.png')
